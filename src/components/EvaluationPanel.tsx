...
export default function EvaluationPanel({
  ...
  const handleGeminiAIDraft = async () => {
    ...
    try {
      ...
      console.error(err);
      setAiError(err?.message || "Verify your API configuration in the Secrets panel.");
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
      ...
      <header className="flex justify-between items-center w-full px-10 z-40 h-16 bg-white border-b border-[#c5c5d3] shrink-0">
        <div className="flex items-center gap-6">
          <span className="text-lg font-bold text-[#00236f]">EduManage Evaluation Panel</span>
          <div className="h-8 w-px bg-[#c5c5d3]"></div>
          
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-[#444651]">Student:</span>
            <select
              value={selectedStudentId}
              onChange={(e) => setSelectedStudentId(e.target.value)}
              className="bg-white border border-[#c5c5d3] rounded px-2.5 py-1 text-xs font-bold text-[#00236f] focus:outline-none"
            >
              {students.map(s => (
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-[#444651]">Task:</span>
            <select
              value={selectedAssignmentId}
              onChange={(e) => setSelectedAssignmentId(e.target.value)}
              className="bg-white border border-[#c5c5d3] rounded px-2.5 py-1 text-xs font-bold text-[#0b1c30] focus:outline-none"
            >
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-[#f8f9ff]">
        ...
        <section className="flex-1 h-full overflow-y-auto bg-[#cbdbf5]/30 p-8 flex flex-col items-center">
          {activeAssignment && activeAssignment.documentTitle ? (
            <div className="max-w-3xl w-full bg-white shadow-sm border border-[#c5c5d3] min-h-[900px] p-12 relative flex flex-col justify-between rounded-md">
              <div>
                {/* Paper header */}
                <div className="mb-8 border-b-2 border-[#00236f] pb-4">
                  <h1 className="text-2xl font-bold text-[#0b1c30] mb-2">{activeAssignment.documentTitle}</h1>
                  <div className="flex justify-between text-xs text-[#444651] font-semibold">
                ...
                {/* Paper Content */}
                <div className="space-y-6 text-[#0b1c30] text-sm leading-relaxed text-justify font-serif">
                  {/* Highlights paragraph */}
                  <p className="relative group bg-[#6df5e1]/20 border-l-4 border-[#14B8A6] p-3 rounded font-serif italic text-sm">
                    ...
                    <span className="block text-[10px] font-sans font-bold text-[#14B8A6] mt-1 uppercase">★ Active Assessment Target Selection</span>
                  </p>
                  ...
                  {activeAssignment.id === 'ethics-thesis' && (
                    <div className="py-6 border-y border-[#eff4ff]">
                      <img 
                        ...
                        className="w-full h-48 object-cover rounded border border-[#c5c5d3] shadow-sm"
                      />
                      <p className="text-center text-[11px] text-[#444651] font-semibold italic mt-2">
              ...
              {/* Page Counter footer */}
              <div className="text-center text-xs text-[#444651] font-bold border-t border-[#eff4ff] pt-4 mt-12 font-mono">
                Page 1 of {activeAssignment.id === 'ethics-thesis' ? '24' : '3'}
              </div>
            </div>
          ) : (
            <div className="m-auto text-center space-y-4 max-w-sm">
              <FileText className="w-16 h-16 text-[#757682] mx-auto opacity-40 animate-pulse" />
              <h3 className="text-lg font-bold text-[#0b1c30]">No Submission Selected</h3>
              <p className="text-xs text-[#444651]">This assignment does not require a written document, or the student has not submitted any drafts yet.</p>
            </div>
          )}
        </section>

        {/* Right Side: Evaluation Forms */}
        <section className="w-full md:w-[450px] lg:w-[480px] h-full bg-white border-l border-[#c5c5d3] overflow-y-auto p-6 space-y-6 flex flex-col shrink-0">
          
          {/* AI grading quick helper */}
          <div className="bg-[#eff4ff] border border-[#dce9ff] p-4 rounded-xl space-y-3">
            ...
            <button
              ...
              className={`w-full py-2 px-4 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${
                isGenerating 
                  ? 'bg-white text-[#1E3A8A] border border-[#d3e4fe]'
                  : (!activeAssignment?.documentBody)
                    ? 'bg-[#c5c5d3] text-[#757682] cursor-not-allowed'
                    : 'bg-[#1E3A8A] hover:bg-[#00236f] text-white cursor-pointer active:scale-95 duration-200'
              }`}
            >
              {isGenerating ? (
                <>
                  <Loader className="w-4 h-4 animate-spin text-[#1E3A8A]" />
            ...
            {isGenerating && (
              <p className="text-[11px] text-[#00236f] text-center font-bold animate-pulse">
                {aiStepMessage}
              </p>
            )}
          </div>
          ...
          {/* Score inputs */}
          <div>
            <h3 className="text-xs font-bold text-[#0b1c30] uppercase tracking-wider mb-3">Numerical Evaluation</h3>
            <div className="flex items-end gap-3 bg-[#eff4ff] p-5 rounded-xl border border-[#c5c5d3]">
              <div className="flex-1">
                <label className="text-[10px] font-bold text-[#444651] mb-1 block uppercase">Final Score (0-100)</label>
                <input 
                  type="number"
                  value={score}
                  onChange={(e) => handleScoreChange(parseInt(e.target.value) || 0)}
                  className="w-full text-3xl font-bold bg-transparent border-b-2 border-[#1e3a8a] focus:border-[#14B8A6] outline-none text-[#0b1c30]"
                />
              </div>
              <div className="text-sm font-semibold text-[#444651] shrink-0">/ 100</div>
              
              {/* Recalculated badge */}
              <div className="px-3.5 py-1.5 bg-[#6df5e1] text-[#006f64] font-extrabold text-sm rounded-full flex items-center gap-1">
                <span>★</span>
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-xs font-bold text-[#0b1c30] uppercase tracking-wider">Evaluation Rubric</h3>
              <span className="text-xs font-semibold text-[#444651]">{completedRubricsCount}/5 Completed</span>
            </div>
                ...
                return (
                  <label 
                    ...
                    className={`flex items-center gap-3 p-3 rounded-lg border ${
                      item.border || 'border-[#c5c5d3]'
                    } hover:border-[#1e3a8a] cursor-pointer transition-colors group`}
                  >
                    <input 
                      type="checkbox"
                      checked={isActive}
                      onChange={() => toggleRubric(item.key as keyof typeof rubrics)}
                      className="w-4 h-4 text-[#1e3a8a] border-[#c5c5d3] rounded focus:ring-[#1E3A8A]"
                    />
                    <span className={`text-xs font-semibold transition-colors ${
                      isActive ? 'text-[#0b1c30]' : 'text-[#444651]'
                    }`}>
                      {item.label}
                    </span>
                  </label>
          ...
          <div className="flex-1 flex flex-col min-h-[160px]">
            <h3 className="text-xs font-bold text-[#0b1c30] uppercase tracking-wider mb-2">Qualitative Feedback</h3>
            <textarea
              value={feedback}
              onChange={(e) => setFeedback(e.target.value)}
              className="flex-1 w-full rounded-xl border border-[#c5c5d3] p-4 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-[#00236f] transition-all resize-none text-[#0b1c30] placeholder-gray-400 leading-relaxed"
          ...
          <div className="grid grid-cols-2 gap-3 shrink-0 pt-2 pb-4">
            <button 
              ...
              className="flex items-center justify-center gap-1.5 border-2 border-[#14B8A6] text-[#14B8A6] py-2.5 rounded-lg text-xs font-bold hover:bg-[#14B8A6]/5 transition-all outline-none active:scale-95 duration-200 cursor-pointer"
            >
            ...
            <button 
              ...
              className="flex items-center justify-center gap-1.5 bg-[#1E3A8A] text-white py-2.5 rounded-lg text-xs font-bold hover:bg-[#00236f] transition-all outline-none active:scale-95 duration-200 cursor-pointer shadow-sm"
            >
